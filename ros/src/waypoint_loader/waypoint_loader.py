...
class WaypointLoader(object):
    ...
    def __init__(self):
        ...
        rospy.logdebug("WaypointLoader: velocity= %s", self.velocity)
        rospy.spin()

    def new_waypoint_loader(self, path):
        rospy.logdebug("new_waypoint_loader: path= %s", path)
        if os.path.isfile(path):
            waypoints = self.load_waypoints(path)
            self.publish(waypoints)
            rospy.loginfo("[WaypointLoader] waypoints_cb: len(waypoints)= %s", len(waypoints))
            #  Waypoint Loded: len(waypoints)= 10902
